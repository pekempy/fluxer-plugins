...
export async function saveConfigData(config) {
    try {
                    ...
                    if (Array.isArray(connections)) {
                        for (const conn of connections) {
                            if (conn.connection_type === 'domain' && conn.verified) {
                                ...
                                const mapping = config.domainMappings.find(m => conn.name.toLowerCase().includes(m.domain.toLowerCase()) ||
...
export async function getUserTags(userIds) {
    const tagsMap = {};
    if (userIds.length === 0)
        return tagsMap;
    ...
    try {
        ...
        const result = await client.query(`SELECT row_key::jsonb ->> 'value' AS user_id, row_data FROM "${client.kvTable()}" WHERE table_name = $1 AND row_key::jsonb ->> 'value' = ANY($2)`, ['users', userIds]);
        ...
        try {
            ...
            const result = await client.execute('SELECT user_id, username, discriminator FROM users WHERE user_id IN ?', [bigIntIds], { prepare: true });
            for (const row of result.rows) {
