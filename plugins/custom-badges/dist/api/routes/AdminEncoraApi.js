...
function createDomainConnectionId(userId, identifier) {
    return createHash('sha256')
        .update(`${String(userId)}:${identifier.toLowerCase()}`)
        .digest('hex');
}
export function mapConnectionToResponse(row) {
    return {
...
export default createRoute({
    ...
    routes: (app) => {
        ...
        app.post('/admin/users/link-encora', async (ctx) => {
            ...
            const acls = ctx.get('adminUserAcls') || ctx.get('adminApiKeyAcls') || adminUser.acls || new Set();
            ...
            const gateway = ctx.get('gatewayService');
        ...
        app.delete('/admin/users/unlink-encora', async (ctx) => {
            ...
            const connections = await repository.findByUserId(userId);
            const encoraConnection = connections.find((c) => c.connection_type === 'domain' && c.name.includes('encora.it'));
            if (encoraConnection) {
...
//# sourceMappingURL=AdminEncoraApi.js.map